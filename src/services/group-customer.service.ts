...
export class GroupCustomerService {
  ...
  static async updateGroupCustomer(id: string, data: UpdateGroupCustomerInput['body'], userId: string): Promise<GroupCustomer | null> {
    try {
  ...
  static async deleteGroupCustomer(id: string, userId: string): Promise<GroupCustomer | null> {
    try {
      const existingGroupCustomer = await prisma.groupCustomer.findUnique({
        where: { id },
      });
