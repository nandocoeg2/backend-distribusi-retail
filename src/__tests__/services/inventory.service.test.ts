...
describe('Inventory Service', () => {
  ...
  describe('getAll', () => {
    it('should return paginated inventories', async () => {
      ...
      const result = await InventoryService.getAll(1, 10);
  ...
  describe('search', () => {
    it('should return inventories matching the query with pagination', async () => {
      ...
      const result = await InventoryService.search(query, 1, 10);
      expect(result).toEqual({
        data: mockInventories,
        pagination: {
          currentPage: 1,
  ...
  describe('getById', () => {
    it('should return an inventory by id with audit trails', async () => {
      const mockInventory = {
        id: '1',
        plu: 'PLU001',
        nama_barang: 'Test Item',
    ...
    it('should throw AppError if inventory not found', async () => {
        (prisma.inventory.findUnique as jest.Mock).mockResolvedValue(null);
        await expect(InventoryService.getById('1')).rejects.toThrow(new AppError('Inventory not found', 404));
    });
  });
