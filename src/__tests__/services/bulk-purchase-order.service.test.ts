...
describe('BulkPurchaseOrderService', () => {
  ...
  describe('processPendingFiles', () => {
    const statuses = {
        pending: { id: 'status_pending', status_code: 'PENDING' },
        processed: { id: 'status_processed', status_code: 'PROCESSED' },
        failed: { id: 'status_failed', status_code: 'FAILED' },
    };

    beforeEach(() => {
        (prisma.status.findUnique as jest.Mock)
            .mockImplementation((options) => {
                if (options.where.status_code === 'PENDING') return statuses.pending;
                if (options.where.status_code === 'PROCESSED') return statuses.processed;
                if (options.where.status_code === 'FAILED') return statuses.failed;
                return null;
            });
    });
    ...
    it('should log an error and abort if core statuses are not found', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('Core statuses (PENDING, PROCESSED, FAILED) not found. Aborting.');
      expect(prisma.$transaction).not.toHaveBeenCalled();
    });
  ...
  describe('getBulkUploadStatus', () => {
    it('should return file status if file is found', async () => {
      ...
      const fileData = { id: fileId, status: { status_code: 'PROCESSED' } };
